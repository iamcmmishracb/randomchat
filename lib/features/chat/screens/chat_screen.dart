...
class ChatScreen extends StatefulWidget {
  ...
  const ChatScreen({super.key, required this.sessionId, required this.partnerName, this.partnerGender});
...
class _ChatScreenState extends State<ChatScreen> with TickerProviderStateMixin {
  ...
  // Live count display (same random base as matching screen)
  ...
  void initState() {
    ...
    _sessionSub = _service.sessionStream.listen((session) {
      if (session == null && _chatEnded == false) {
        ...
        builder: (ctx, setModal) => Padding(
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                Text('Report User', style: Theme.of(context).textTheme.headlineSmall),
                const SizedBox(height: 4),
                Text('Your report is anonymous and reviewed by our team.', style: Theme.of(context).textTheme.bodySmall),
                const SizedBox(height: 16),

                if (_selectedCategory == null) ...[
                  ...['Harassment / Abuse', 'Inappropriate Content', 'Spam / Bot', 'Underage User', 'Hate Speech', 'Other'].map((cat) =>
                    ListTile(
                      contentPadding: EdgeInsets.zero,
                      leading: const Icon(Icons.flag_outlined, color: AppColors.error, size: 18),
                      title: Text(cat, style: Theme.of(context).textTheme.bodyLarge),
                      trailing: const Icon(Icons.chevron_right_rounded, color: AppColors.textMuted, size: 18),
                      onTap: () => setModal(() => _selectedCategory = cat),
                    ),
                  ),
                ] else ...[
                  Row(children: [
                    GestureDetector(
                      onTap: () => setModal(() => _selectedCategory = null),
                      child: const Icon(Icons.arrow_back_rounded, color: AppColors.textSecondary, size: 20),
                    ),
                    const SizedBox(width: 8),
                    Expanded(child: Text(_selectedCategory!, style: Theme.of(context).textTheme.titleMedium?.copyWith(color: AppColors.error))),
                  ]),
                  const SizedBox(height: 14),
                  Text('Add details (optional)', style: Theme.of(context).textTheme.titleMedium),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        Navigator.pop(ctx);
                        AppUtils.showSnackBar(context, 'Report submitted. Thank you for keeping RandomChat safe.');
                      },
        ...
        children: [
          Column(
            children: [
              ...
              if (_chatEnded) _buildDisconnectMessage(),
              ChatInputBar(
  ...
  PreferredSizeWidget _buildAppBar() {
    return AppBar(
      ...
      leadingWidth: 56,
      leading: IconButton(onPressed: () => context.go(AppRoutes.home), icon: const Icon(Icons.home_outlined)),
      title: Row(
        children: [
          AvatarWidget(name: widget.partnerName, gender: widget.partnerGender, size: 36, fontSize: 14),
          const SizedBox(width: 10),
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(widget.partnerName, style: Theme.of(context).textTheme.titleMedium),
              Row(children: [
                Container(
                  width: 6, 
                  height: 6, 
      ...
      actions: [
        Container(
          ...
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            Container(width: 5, height: 5, decoration: const BoxDecoration(color: AppColors.online, shape: BoxShape.circle)),
            const SizedBox(width: 4),
            Text('${(_liveCount / 1000).toStringAsFixed(1)}k', style: const TextStyle(color: AppColors.online, fontSize: 11, fontWeight: FontWeight.w600)),
          ]),
        ),
        PopupMenuButton<String>(
          ...
          itemBuilder: (_) => [
            PopupMenuItem(value: 'report', child: Row(children: [const Icon(Icons.flag_outlined, color: AppColors.error, size: 18), const SizedBox(width: 8), Text('Report User', style: TextStyle(color: Theme.of(context).textTheme.bodyLarge?.color))])),
          ],
        ),
      ],
    );
  ...
  Widget _buildMessageList() {
    if (_messages.isEmpty) {
      return Center(child: Column(
        ...
        children: [
          ...
          Text('Connected! Say hello 👋', style: Theme.of(context).textTheme.bodyMedium),
        ],
      ));
    }
    return ListView.builder(
      ...
      itemBuilder: (_, i) {
        ...
        final showTimestamp = i == 0 || _messages[i].sentAt.difference(_messages[i - 1].sentAt).inMinutes > 5;
  ...
  Widget _buildTypingIndicator() {
    return Padding(
      ...
      child: Row(
        children: [
          AvatarWidget(name: widget.partnerName, gender: widget.partnerGender, size: 24, fontSize: 10),
          const SizedBox(width: 8),
          Container(
              ...
              children: List.generate(3, (i) => AnimatedBuilder(
                ...
                builder: (_, __) {
                  ...
                  return Container(
                    ...
                    width: 6,
                    height: 6,
                    decoration: BoxDecoration(shape: BoxShape.circle, color: AppColors.textMuted.withOpacity(0.4 + val * 0.6)),
                  );
                },
              )),
            ),
  ...
  Widget _buildDisconnectMessage() {
    return Container(
